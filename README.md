...
Add your own screenshots to the `assets/` folder, then update the image paths below as needed.
...
Example embeds (replace with your captures):

![Pages View](assets/pages.png)

![Cookies Dump](assets/cookies.png)

## Demo GIF

Record a short terminal demo of listing pages and dumping cookies, then place it at `assets/demo.gif`.

![CrumbBum Demo](assets/demo.gif)

### Capture Tips

- macOS (GUI): Use QuickTime Player → New Screen Recording. Export as `.mov`, convert to GIF with `ffmpeg`:
  - `ffmpeg -i demo.mov -vf "fps=10,scale=1024:-1:flags=lanczos" -loop 0 assets/demo.gif`
- Linux/Windows: Use OBS Studio to record a window/region. Convert with `ffmpeg` as above.
- Terminal-only (cross-platform): Use `asciinema` to record and then render to GIF (e.g., `agg`), or keep the asciinema cast in the repo:
  - `asciinema rec demo.cast` → run `crumbbum` commands → `Ctrl-D`
  - Render (if you have `agg`): `agg -i demo.cast -o assets/demo.gif`
